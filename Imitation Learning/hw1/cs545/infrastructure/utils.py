...
def sample_trajectory(env, policy, max_path_length, render=False):
    ...
    while True:
        ...
        obb = ptu.from_numpy(np.array([ob])[None,:])
        ac = ptu.to_numpy(policy.forward(obb).sample().squeeze(0))
        ac = ac[0]
        ...
        steps += 1
        rollout_done = 1 if done or (steps == max_path_length) else 0 # this is either 0 or 1
...
def sample_trajectories(env, policy, min_timesteps_per_batch, max_path_length, render=False):
    ...
    print("mtpb: ", min_timesteps_per_batch)
