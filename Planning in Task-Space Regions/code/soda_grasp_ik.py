...
def createBw():
    ...
    Bw = np.array([
        [-0.05, 0.05],  # x_min, x_max
        [-0.05, 0.05],  # y_min, y_max
        [0.0, 0.0],     # z_min, z_max (fixed height)
        [0.0, 0.0],     # _min, _max (no rotation about x-axis)
        [0.0, 0.0],     # _min, _max (no rotation about y-axis)
        [-np.pi, np.pi] # _min, _max (allow full rotation about z-axis)
    ])
...
def main(if_sim):
    ...
    for configuration in configurations:
        ...
        adarrt = AdaRRT(start_state=np.array(arm_home), goal_state=np.array(configuration), ada=ada, ada_collision_constraint=full_collision_constraint)
    ...
    q_curr = arm_skeleton.get_positions()
    # dq = np.transpose(J) @ np.linalg.pinv(J @ np.transpose(J)) @ dx
    dq = np.dot(np.transpose(J), np.dot(np.linalg.pinv(np.dot(J, np.transpose(J))), dx))
    q = q_curr - dq
    ...
    #Q7
    # q_start =  arm_skeleton.get_positions()
    # q_goal = q_start + [0,0,0,-0.5,0,0]
    # dx = [0,0,0,-0.01,0,0]
    # q_curr = arm_skeleton.get_positions()
    # while np.linalg.norm(q_curr - q_goal):
    #     J = arm_skeleton.get_jacobian(hand.get_endeffector_body_node())
    #     dq = np.transpose(J) @ np.linalg.pinv(J @ np.transpose(J)) @ dx
    #     q = q_curr - dq
